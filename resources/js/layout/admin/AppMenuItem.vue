...
<template>
    <li :class="{
        ...
        <a v-if="
            ...
            item.visible !== false &&
            canNavigate(item.to)
        " :href="item.url" @click="itemClick($event, item, index)" :class="item.class" :target="item.target" tabindex="0" :style="{
        ...
        <router-link v-if="
            ...
            item.visible !== false &&
            canNavigate(item.to)
        " @click="itemClick($event, item, index)" :class="[item.class, { 'active-route': checkActiveRoute(item) }]" tabindex="0" :to="item.to" :style="{
            '--nav-color': item.color,
        }">
            <i :class="item.icon" class="layout-menuitem-icon"></i>
            <span class="layout-menuitem-text">{{ item.label }}</span>
