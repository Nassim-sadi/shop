            ...
            <div style="
                ...
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    ...
                    <div>
                        ...
                        <InputText id="email1" type="text" :placeholder="$t('email.placeholder')" class="w-full md:w-[30rem] mb-4" fluid v-model="email" />

                        <div class="text-red-500" v-for="error of v$.email.$errors" :key="error.$uid">
                            <Message severity="error">{{
                                error.$message
                            }}</Message>
                        </div>

                        <label for="password1" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2 mt-4">{{ $t("password") }}</label>
                        <Password id="password1" v-model="password" :placeholder="$t('password')" :toggleMask="true" class="mb-4" fluid :feedback="false"></Password>

                        <div class="text-red-500" v-for="error of v$.password.$errors" :key="error.$uid">
                            <Message severity="error">{{
                                error.$message
                            }}</Message>
                        </div>
                        ...
                        <div class="flex items-center justify-between mt-2 mb-8 gap-8">
                            <div class="flex items-center">
                                <Checkbox v-model="checked" id="remember" binary class="mr-2"></Checkbox>
                                <label for="remember">{{ $t("remember_me") }}
                                </label>
                            </div>

                            <Button link @click="forgotPassword" :label="$t('auth.forgot_password') + ' ?'">
                            </Button>
                        </div>
                        <Button :label="$t('auth.sign_in')" class="w-full" @click="login" :loading="loading"></Button>
                    </div>
                </div>
            </div>
        </div>
